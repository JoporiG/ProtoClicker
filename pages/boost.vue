...
export default {
    ...
    methods: {
        newBoost(id){
            ...
            if(this.$store.getters.getCoins >= this.boosts[id].price){//проверка на деньги
                ...
                this.$store.dispatch('addAmmount', id);
            } else {
    ...
    mounted(){
            let NewBoosts = this.$store.getters.getAmmount;
            for(let i = 0;  i < this.boosts.length; i++){
                this.boosts[i].ammount = parseInt(NewBoosts[i]);
            }
        }
}
</script>
